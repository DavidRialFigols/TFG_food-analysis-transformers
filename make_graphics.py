...
def graphics_diff_epochs(models_dict, using_transformer, mat_size, num_blocks, fig_size, layer=False):
    ...
    for n in range(num_blocks):
        ...
        for l in range(1, len(q11)):
            ...
            print(0.05*abs(torch.sum(q11[l]-q11[l-1]).item())/(mat_size*mat_size))
            print(torch.std(q11[l]-q11[l-1], unbiased=False))
            print(max(0.05*abs(torch.sum(q11[l]-q11[l-1]).item())/(mat_size*mat_size), torch.std(q11[l]-q11[l-1], unbiased=False)))
            print()
